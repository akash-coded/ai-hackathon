...
import { getQuickSummary, isApiKeyConfigured } from '../services/geminiService';

export default function BusinessDashboard() {
  const navigate = useNavigate();
  const [loadingSummary, setLoadingSummary] = useState<string | null>(null);
  const [activeSummary, setActiveSummary] = useState<{ ticketId: string; text: string } | null>(null);

  const handleQuickSummary = async (ticketId: string) => {
    if (!isApiKeyConfigured()) {
      setActiveSummary({
        ticketId,
        text: 'Please configure your Gemini API key in the .env file to use AI features.'
      });
      return;
    }
    ...
    try {
      ...
      setActiveSummary({ ticketId, text: summary });
    } catch (error) {
      setActiveSummary({
        ticketId,
